...
namespace online_hospital
{
    public class Patient
    {
        ...
        public Patient(string proprieetati)
        {
            ...
            _firstName = token[1];
            _lastName = token[2];
            _healthProblem = token[3];
            _degreeProblem = token[4];
            _dateHospitalization = int.Parse(token[5]);
            _idDoctorPatient = int.Parse(token[6]);
        }

        public Patient(int idPatient, string firstName, string lastName, string healthProblem, string degreeProblem, int dateHospitalization, int idDoctorPatient)
        {
        ...
        {
            return this._idPatient + "," + this._firstName + "," + this._lastName + "," + this._healthProblem + ","  + this._degreeProblem + "," + this._dateHospitalization + "," + this._idDoctorPatient;
        }
    }
}
